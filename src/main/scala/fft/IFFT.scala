...
class IFFT(val numPoints: Int) extends Module {
  require(isPow2(numPoints), s"IFFT size must be a power of 2, got $numPoints")
  ...
  val io = IO(new Bundle {
    ...
    val mode  = Input(Bool())  // false => do /N, true => no /N
  })

  // Parameters
  private val logN = log2Ceil(numPoints)
  private val TWIDDLE_BP = 16  // Must match FFT precision
  ...
  val idle :: prepare :: compute :: normalize :: done :: Nil = Enum(5)
  ...
  switch(state) {
    is(idle) {
      when(io.start) {
        // Load data and take conjugate, with prescaling
    ...
    is(normalize) {
      // Final conjugate and normalization
      for (i <- 0 until numPoints) {
        val finalReal = dataRegs(i).real
        val finalImag = -dataRegs(i).imag  // Take conjugate again
        
        // Apply final scaling based on mode
        val scaleShift = Mux(io.mode, 1.U, logN.U)
        dataRegs(i).real := roundShift(finalReal, scaleShift)
        dataRegs(i).imag := roundShift(finalImag, scaleShift)
      }
      state := done
    }
  ...
  def butterfly(a: ComplexNum, b: ComplexNum, w: ComplexNum): (ComplexNum, ComplexNum) = {
    ...
    val bwr = (b.real * w.real) >> (TWIDDLE_BP - 1)
    ...
    // Butterfly addition/subtraction with wrapping protection
